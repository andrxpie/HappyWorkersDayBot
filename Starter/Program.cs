...
List<long> adminsChatId = new List<long>{807491885};
        ...
        if (!adminsChatId.Contains(message.Chat.Id))
        {
            ...
            switch (message.Type)
            {
                case MessageType.Text:
                    Console.WriteLine($"{message.Chat.Username}[{message.Chat.Id}] | {message.Text}");
...
async Task<IEnumerable<TelegramUserDto>> GetUsersAsync(bool admin =  false)
{
    ...
    List<TelegramUserDto> result = new List<TelegramUserDto>();
